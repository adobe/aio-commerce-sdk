...
const STATIC_APP_CONFIG_IMPORT = [
...
export async function generateRuntimeAppConfigModule() {
  const projectRoot = await getProjectRootDirectory();
  const runtimeConfigPath = getRuntimeAppConfigPath();
  const outputDir = await makeOutputDirFor(dirname(runtimeConfigPath));
...
/** Check whether generated actions must import the runtime-safe config module. */
function usesRuntimeAppConfigModule(appManifest: CommerceAppConfigOutputModel) {
  return hasDynamicBusinessConfigSchema(appManifest);
}

/** Switch static JSON imports to the generated app config module for dynamic schemas. */
function applyRuntimeAppConfigImport(template: string, actionName: string) {
  const runtimeConfigImport = `import appConfig from "${APP_CONFIG_IMPORT_ALIAS}";`;

  if (actionName === "app-config") {
    return template
      .replace(STATIC_APP_CONFIG_IMPORT, runtimeConfigImport)
      .replace(
  ...
  if (actionName === "config") {
    return template
      .replace(STATIC_CONFIG_SCHEMA_IMPORT, runtimeConfigImport)
      .replace(
...
/** Prepare config artifacts used by generated runtime actions. */
export async function prepareRuntimeAppConfigModule(
  appManifest: CommerceAppConfigOutputModel,
) {
  if (usesRuntimeAppConfigModule(appManifest)) {
...
export async function generateActionFiles(
  ...
  consola.start(`Generating runtime actions for ${extensionPointId}...`);

  const useRuntimeAppConfigModule = usesRuntimeAppConfigModule(appManifest);
