...
export async function updateExtConfig(
  ...
  await writeHookFiles(
    Object.keys(extConfig.hooks ?? {}),
    extensionPointId,
    projectRoot,
  );
...
/**
 * Writes the CommonJS files that an extension point's `ext.config.yaml` hooks point to.
 * @param hookNames - Names of the hooks registered for the extension point.
 * @param extensionPointId - Extension point that owns the hooks.
 * @param projectRoot - Resolved project root where the files are generated.
 */
async function writeHookFiles(
  ...
  const extension = extensionPointId.replace("commerce/", "");

  await Promise.all(
    hookNames.map(async (hookName) => {
      const hookPath = getHookPath(extensionPointId, hookName);
      const handlerCall = HOOK_HANDLER_CALLS[hookName](extension);

      await makeOutputDirFor(dirname(hookPath), projectRoot);
      await writeFile(
        join(projectRoot, hookPath),
        [
          "// This file has been auto-generated by `@adobe/aio-commerce-lib-app`",
          "// Do not modify this file directly",
          "",
