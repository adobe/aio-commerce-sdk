...
Hook files can be `.js` or `.ts`, and must export a function as their default export. The function receives the argument the aio CLI passes to the hook, and an object with the ID of the extension point that runs it:
