...
describe("runtime is configurable per app", () => {
  test("buildAppManagementExtConfig defaults all actions to nodejs:22", () => {
    const result = buildAppManagementExtConfig(configWithCommerceEventing);
    const actions = result.runtimeManifest?.packages?.[PACKAGE_NAME]?.actions;
  ...
  test("buildAppManagementExtConfig propagates runtime from appConfig to all actions", () => {
    const config = {
      ...configWithCommerceEventing,
      runtime: "nodejs:24" as const,
    };
    ...
    for (const action of Object.values(actions ?? {})) {
      expect(action.runtime).toBe("nodejs:24");
    }
  });

  test("buildBusinessConfigurationExtConfig defaults to nodejs:22", () => {
    const result = buildBusinessConfigurationExtConfig();
    const actions = result.runtimeManifest?.packages?.[PACKAGE_NAME]?.actions;
  ...
  test("buildBusinessConfigurationExtConfig uses provided runtime", () => {
    const result = buildBusinessConfigurationExtConfig("nodejs:24");
    const actions = result.runtimeManifest?.packages?.[PACKAGE_NAME]?.actions;
  ...
  test("buildAdminUiSdkExtConfig defaults to nodejs:22", () => {
    const config = buildAdminUiSdkExtConfig();
    const action =
      config.runtimeManifest?.packages?.["admin-ui-sdk"]?.actions?.registration;
  ...
  test("buildAdminUiSdkExtConfig uses provided runtime", () => {
    const config = buildAdminUiSdkExtConfig("nodejs:24");
    const action =
      config.runtimeManifest?.packages?.["admin-ui-sdk"]?.actions?.registration;
