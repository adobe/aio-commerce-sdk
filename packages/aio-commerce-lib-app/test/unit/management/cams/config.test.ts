...
describe("resolveCamsBaseUrl", () => {
  ...
  test("selects the stage host when the environment is stage", () => {
    expect(resolveCamsBaseUrl({}, "stage")).toBe(STAGE_CAMS_BASE_URL);
  });

  test("selects the production host when the environment is prod", () => {
    expect(resolveCamsBaseUrl({}, "prod")).toBe(PROD_CAMS_BASE_URL);
    expect(PROD_CAMS_BASE_URL).toBe(DEFAULT_CAMS_BASE_URL);
  });

  test("an explicit override wins over the environment", () => {
    expect(
      resolveCamsBaseUrl(
        { [CAMS_BASE_URL_INPUT]: "https://cams.local" },
        "stage",
      ),
    ).toBe("https://cams.local");
  });
});
