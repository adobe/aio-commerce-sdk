...
Adds custom columns to the order, product, customer, invoice, credit memo, shipment, or newsletter grid in Commerce Admin.
...
- `gridType` — `"order" | "product" | "customer" | "invoice" | "creditmemo" | "shipment" | "newsletter"` (note: credit memo sends the lowercase `"creditmemo"`, not `"creditMemo"`)
