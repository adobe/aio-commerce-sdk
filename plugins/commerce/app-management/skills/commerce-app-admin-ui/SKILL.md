...
The build derives the extension's `ext.config.yaml` from your `adminUi` config: each worker `runtimeAction` becomes a `workerProcess` operation, and when you declare a `menu` or any `view`-type entry, a `view` operation plus an explicit `web: web-src` key are written. Those `hooks`, `operations`, and `web` sections are managed by the library — do not hand-edit them. To run custom code in a managed hook, add a hook file at `hooks/<hook>.<ext>` (all extension points) or `src/commerce-backend-ui-2/hooks/<hook>.<ext>` (this extension point only) instead; see the lib-app usage docs, "Custom hook code".
