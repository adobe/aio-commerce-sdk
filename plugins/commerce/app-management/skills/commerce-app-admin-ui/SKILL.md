...
description: >
  ...
  order, product, customer, invoice, credit memo, shipment, or newsletter grid, add a
...
- **Which entity** — grid columns support `order`, `product`, `customer`, `invoice`, `creditMemo`, `shipment`, or `newsletter`; mass actions support only `order`, `product`, `customer`, or `newsletter`; view buttons are order-only; menu and custom ACL resources have no entity
...
- **Grid columns are worker-only**: there is no `view` grid column. `order`, `product`, `customer`, `invoice`, `creditMemo`, `shipment`, and `newsletter` support `gridColumns`; only `order`, `product`, `customer`, and `newsletter` support `massActions`; and only `order` supports `viewButtons`.
